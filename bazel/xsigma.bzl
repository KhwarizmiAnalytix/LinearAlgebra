# =============================================================================
# LinearAlgebra — shared Bazel compile/link helpers (standalone)
# =============================================================================
# Copy of XSigma's bazel/xsigma.bzl, trimmed to what this repo uses (the Enzyme
# helpers are XSigma-only and intentionally omitted).
# =============================================================================

def xsigma_copts(cxx_std = "c++20", cstdlib_include = True):
    """Returns common compiler options for LinearAlgebra targets.
    ...
    Args:
        ...
        cstdlib_include: Whether to force-include <cstdlib> on non-Windows (mirrors
                 CMake's `target_compile_options(Parallel PRIVATE -include cstdlib)`).
    """
    return select({
...
def xsigma_linkopts():
    """Returns common linker options for Parallel targets."""
    return select({
        "@platforms//os:windows": [],
        "@platforms//os:macos": [
            "-undefined",
